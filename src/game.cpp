      ...
      random_h(0, static_cast<int>(grid_height - 1))
{
  ...
  {
    food = Food();
    food.SetIsToxic(true);
    food.SetToxicFoodColor();
    PlaceFood(&food);
    toxicFoods.emplace_back(food);
  }
}
               ...
               std::size_t target_frame_duration)
{
  ...
  while (running)
  {
    ...
    Update();
    renderer.Render(snake, foods, toxicFoods);
...
bool Game::HasEatenToxicFood(int player_x, int player_y)
{
  for(Food toxicFood : toxicFoods)
  {
    SDL_Point foodPos = toxicFood.GetPosition();

    // Check if there's food over here
    if (foodPos.x == player_x && foodPos.y == player_y)
    {
      return true;
    }
  }
...
void Game::Update()
{
  ...
  if(HasEatenToxicFood(new_x, new_y)){
    snake.alive = false;
    return; 
  }
